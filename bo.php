...
$sql = mysqli_query($con, "SELECT * FROM (SELECT home.id,home.name,home.img, book.val,book.book,book.ad FROM `book` JOIN home WHERE book.id=home.id AND book.val=1)as x JOIN (SELECT * FROM book1 WHERE uid=$user_id) as p WHERE p.hid=x.id
       ...
       ?>
            ...
            <div class="col-lg-4">
              <div class="card">
                  ...
                  <div class="card-body">
                      <h4 class="card-title display-3"><?= $row['name']; ?> [<?= $row['book']; ?> ]  </h4>
...
<?php  
       ...
       if( $row['book']=='not confirmed'&& $row['val']!=0)
       {
 ?>

         <a href="acti.php?up=<?= $row['id'];?>" class="btn bg-secondary cardbtn" onclick="return confirm('confirm that you want to Cancel this bookig');">Cancel Booking</a>
